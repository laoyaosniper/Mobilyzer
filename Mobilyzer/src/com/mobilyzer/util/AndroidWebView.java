...
public class AndroidWebView extends WebView {
  ...
  public AndroidWebView(Context context, boolean spdyTest, WebViewProtocol protocol,
      long startTimeFilter, String url) {
    super(context);
    ...
    setWebViewClient(new WebViewClient() {
      ...
      public void onPageFinished(WebView view, String url) {
        ...
        newintent.setAction((UpdateIntent.PLT_MEASUREMENT_ACTION)
            + AndroidWebView.this.startTimeFilter);
      ...
      public WebResourceResponse shouldInterceptRequest(WebView view, String urlStr) {
        ...
        if (urlStr == null || urlStr.trim().equals("")
            || !(urlStr.startsWith("http") && !urlStr.startsWith("www")) || urlStr.contains("|")) {
        ...
        try {
          Request request =
              new Request.Builder()
                  ...
                  .header(
                      "User-Agent",
                      "Mozilla/5.0 (Linux; U; Android 4.3; en-us; SCH-I535 Build/JSS15J) AppleWebKit/534.30 (KHTML, like Gecko) Version/4.0 Mobile Safari/534.30")
                  .build();
          ...
          Logger.d("ashkan_plt: HTTP: " + client.getConnectionPool().getHttpConnectionCount()
              + " SPDY: " + client.getConnectionPool().getSpdyConnectionCount());
    ...
    setWebChromeClient(new WebChromeClient() {
      ...
      public void onConsoleMessage(String message, int lineNumber, String sourceID) {
        ...
        if (message.startsWith("mobilyzer_navigation")) {
          ...
          Logger.d("ashkan_plt: onConsoleMessage: Broadcasting mobilyzer_resource results "
              + message.length());
          PhoneUtils.getGlobalContext().sendBroadcast(newintent);

          if (AndroidWebView.this.spdyTest
              && AndroidWebView.this.protocol.equals(WebViewProtocol.HTTP)) {
            AndroidWebView spdyWebView =
                new AndroidWebView(AndroidWebView.this.context, true, WebViewProtocol.SPDY,
                    AndroidWebView.this.startTimeFilter, AndroidWebView.this.url);
            spdyWebView.loadUrl();
          }
