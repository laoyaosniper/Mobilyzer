...
public class PageLoadTimeTask extends MeasurementTask {
  ...
  public MeasurementResult[] call() throws MeasurementError {
        ...
        if (intent.getAction().equals((UpdateIntent.PLT_MEASUREMENT_ACTION) + startTimeFilter)) {
          ...
          if (intent.hasExtra(UpdateIntent.PLT_TASK_PAYLOAD_RESULT_NAV)) {
            String navigationStr =
                intent.getStringExtra(UpdateIntent.PLT_TASK_PAYLOAD_RESULT_NAV).substring(20);
            ...
            String resrourcesStr =
                intent.getStringExtra(UpdateIntent.PLT_TASK_PAYLOAD_RESULT_RES).substring(18);
            ...
            Logger.d("ashkan_plt: >>>>resourceTimingResults: " + resrourcesStr.length() + " "
                + resourceTimingResults.size());
    ...
    if (isDone()) {
      ...
      MeasurementResult result =
          new MeasurementResult(phoneUtils.getDeviceInfo().deviceId,
              phoneUtils.getDeviceProperty(this.getKey()), PageLoadTimeTask.TYPE,
              System.currentTimeMillis() * 1000, TaskProgress.COMPLETED, this.measurementDesc);
      ...
      MeasurementResult result =
          new MeasurementResult(phoneUtils.getDeviceInfo().deviceId,
              phoneUtils.getDeviceProperty(this.getKey()), PageLoadTimeTask.TYPE,
              System.currentTimeMillis() * 1000, TaskProgress.FAILED, this.measurementDesc);
      int nav_index = 0;
    ...
    PhoneUtils.getGlobalContext().stopService(
        new Intent(PhoneUtils.getGlobalContext(), PLTExecutorService.class));
    return mrArray;
